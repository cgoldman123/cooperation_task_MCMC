function [samples, stats, structArray] = matjags_cmg(dataStruct, jagsFilenm, initStructs , varargin)
...
elseif ismac | isunix
    if dataStruct.NS == 1
        defaultworkingDir = [dataStruct.result_dir 'jagstmp_' dataStruct.subject]; % change this to fit your particular needs
    else
        defaultworkingDir = [dataStruct.result_dir 'jagstmp']; % change this to fit your particular needs
    end
end

dataStruct = rmfield(dataStruct, {'result_dir', 'subject'});
...
if doParallel==1
    ...
    elseif ismac | isunix
        parfor whchain=1:nChains
            jagsScript   = sprintf( 'jagscript%d.cmd' , whchain );
            % SMT EDIT BELOW
            if ismac      % IF RUNNING LOCAL
                jagsPrefix = sprintf('/usr/local/bin/');
            elseif isunix % IF RUNNING ON SERVER
